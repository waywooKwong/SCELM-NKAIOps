...
# 将时间转换为字符串格式
# train_start_time_str = train_start_time.strftime('%Y-%m-%d %H:%M:%S')
# train_end_time_str = train_end_time.strftime('%Y-%m-%d %H:%M:%S')
...
# 获得prefix名称
folder_path = '/home/sunyongqian/liuheng/aiops-scwarn/result_json_and_csv_29867/result_csv/29867'  # 替换成你的文件夹路径
...
# 读取JSON文件
with open('/home/sunyongqian/liuheng/aiops-scwarn/result_json_and_csv_29867/result_json/result_29867.json', 'r', encoding='utf-8') as f:
    data = json.load(f)
...
for col_index in range(0, len(input_string)):
    ...
    content.append('    ' + str(col_index + 1) + '. ')
    content.append('指标训练集：' + prefix_trans_map[prefix_list[col_index]] + '; ' + prefix_list[col_index] + ';')  # 源名称
    for p in prompt:
    ...
    prompt = model.forecast(input_data)
    content.append('    ' + str(col_index + 1) + '. ')
    content.append('指标测试集：' + prefix_trans_map[prefix_list[col_index]] + '; ' + prefix_list[col_index] + ';')  # name
    for p in prompt:
        content.append(str(p))
    content.append('\n')
...
df = pd.read_csv('/home/sunyongqian/liuheng/aiops-scwarn/result_json_and_csv_29867/result_csv/29867/overall.csv')
...
df = pd.read_csv('/home/sunyongqian/liuheng/Time-Series-Library-main/AIOps_dataset/fluxrankplus/src/fluxrankplus/model/combined29867.csv')
...
for idx, row in df.iterrows():
    ...
    if csvname == current_csvname:
        ...
        if current_csvname is not None:
            anomaly_info[-1]['summary'] = "\n      该指标共出现 {} 异常类型，类型为：{}\n".format(pattern_count, pattern_list)
            timestamp_count = 0
            pattern_count = 0
            pattern_list = []
...
# 将内容写入文件
with open('/home/sunyongqian/liuheng/Time-LLM/weihua/CrossAttention-demo/areaText_29867.txt', 'a') as file:
