...
def AreaText_Part2(target_index, suffix):
    ...
    text_path = (
        "/home/zhengtinghua/kuangweihua/SCWarn_kontrast/areaText/Chinese/areaText_"
        + index
        + "_v4.txt"
    )
    overall_csv_path = (
        "/home/zhengtinghua/shenchao/aiops-scwarn/new_res/emailservice/result_json_and_csv_"
        + index
        + "/result_csv/"
        + index
        + "/overall.csv"
    ...
    combine_csv_path = (
        "/home/zhengtinghua/kuangweihua/SCWarn_kontrast/process_mid_data/k4_combine/combined_"
        + index
        + ".csv"
    )
        ...
        for pattern, timestamps in pattern_timestamp_map.items():
            ...
            if pattern in {
                ...
                if overlapping_timestamps:
                    ...
                    content.append(
                        "\n      其中，类型[{}] 恢复正常，与整体同时出现异常的时间戳: {}".format(
    ...
    for col_index in range(0, len(input_string)):
        ...
        # 将数据转换为数值并处理错误 2024/11/02
        print("input_data before:",input_data)
        input_data = pd.to_numeric(input_data, errors='coerce').dropna()
        input_data = torch.tensor(input_data.values).view(1, len(input_data), 1)
        print("input_data: after",input_data)
