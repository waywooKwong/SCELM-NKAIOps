...
for kpi in kpis:
    ...
    embeddings = HuggingFaceEmbeddings(
        model_name="kuangweihua/ollama/models/m3e-base", model_kwargs={"device": "cuda"}
    )
    ...
    for id in ids:
        ...
        value_set = r.smembers(target_key)
        doc_from_redis = []
        ...
        root_cause_from_redis = r.smembers(str("root:" + anomaly_cluster_str))
        ...
        print("root reason2:(get from redis according cluster)", str(root_cause_from_redis))
        
        r1 = redis.Redis(host="localhost", port=6379, db=1)
        if add_to_redis == True:
            print("====")
            r1.sadd(str("root:" + anomaly_cluster_str), classification_response)
        ...
        if add_to_redis == True:
            r1.sadd(str("cluster:" + str(cluster_label)), content)
