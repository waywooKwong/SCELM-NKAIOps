...
class MLSTM(nn.Module):
    def __init__(self, input_dim1, input_dim2, input_dim3, hidden_dim, batch_size=64):
        ...
        self.lstm3 = nn.LSTM(input_dim3, hidden_dim, batch_first=True)

        self.hiddent1out = nn.Linear(hidden_dim*2, input_dim1)
        self.hiddent2out = nn.Linear(hidden_dim*2, input_dim2)
        self.hiddent3out = nn.Linear(hidden_dim*2, input_dim2)
        # hidden
        self.hiddenout = nn.Linear(hidden_dim*2, input_dim1+input_dim2+input_dim3)
    ...
    def forward(self, seq1, seq2, seq3):
        ...
        lstm_out3, _ = self.lstm3(seq3.view(self.batch_size, -1, self.input_dim3))
        shared = torch.cat((lstm_out1, lstm_out2, lstm_out3), 2)
        ...
        return predict[:, -1, :], (lstm_out1[:, -1, :], lstm_out2[:, -1, :], lstm_out3[:-1:])


def train(dataloader, modal, batch_size, n_epoch, lr=0.01):
    input_dim1, input_dim2, input_dim3 = modal[0], modal[1], modal[2]
    print("input dim1, input dim2", input_dim1, input_dim2, input_dim3)
    model = MLSTM(input_dim1, input_dim2, input_dim3, 48, batch_size)
    ...
    for epoch in range(n_epoch):
        ...
        for step, (batch_X, batch_Y) in enumerate(dataloader):
            ...
            predicted, (H1, H2) = model(batch_X[:,:, :input_dim1], batch_X[:, :, input_dim1: input_dim1+input_dim2], batch_X[:, :, input_dim1+input_dim2: input_dim1+input_dim2+input_dim3])
...
def predict(model, test_data, seq_len, modal):
    ...
    with torch.no_grad():
        for i in range(seq_len, len(test_data)):
            ...
            predicted, _ = model(seq[:,:modal[0]], seq[:,modal[0]:modal[0]+modal[1]], seq[:,modal[0]+modal[1]:modal[0]+modal[1]+modal[2]])
