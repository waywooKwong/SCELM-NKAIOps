...
def AreaText_Part2_English(target_index, suffix):
    ...
    # 5.
    text_path = '/home/sunyongqian/liuheng/Time-LLM/weihua/AreaText/data/areaText_v5/english/areaText_'+index+'_v4_English.txt'
    ...
    for col_index in range(0, len(input_string)):
        ...
        input_data = torch.tensor(input_data.values).view(1, len(input_data), 1)  # Convert to default shape
        content.append('    ' + str(col_index + 1) + '.kpi name: '+ prefix_trans_map[prefix_list[col_index]])
    ...
    for idx, row in df.iterrows():
        ...
        if csvname == current_csvname:
            ...
            if current_csvname is not None:
                anomaly_info[-1]['summary'] = "\n      This kpi has {} anomaly types in total, with types: {}".format(pattern_count, pattern_list)
                timestamp_count = 0
                pattern_count = 0
                pattern_list = []
            ...
            if csvname in prefix_trans_map:
                anomaly_info.append({
                    'header': "\n    {}.kpi name: {}".format(exception_count, prefix_trans_map[csvname]),
        ...
        for pattern, timestamps in pattern_timestamp_map.items():
            if pattern in {type_dict[7], type_dict[8], type_dict[9], type_dict[10], type_dict[11], type_dict[12], type_dict[13]}:  # Check for patterns 7 to 13
                ...
                if overlapping_timestamps:
                    ...
                    content.append("\n      Among them, type [{}] recovered to normal, timestamps coinciding with overall anomalies: {}".format(pattern, ', '.join(map(str, overlapping_timestamps))))
            else:
                # Print all timestamps for patterns 1 to 6
                content.append(info['header'])
                if 'summary' in info:
                    content.append(info['summary'])    
                content.append("\n      Among them, type [{}] remained abnormal, timestamps are: {}".format(pattern, ', '.join(map(str, timestamps))))
    ...
    text = """These are definitions for anomaly types (patterns):
    ...
    with open(text_path, 'a') as file:
        ...
        file.write("Total number of anomaly description shapes: {}\n\n".format(unique_count))
        file.write(text)    
        ...
        file.write("Also provide your thoughts and reasoning. If it's a failed change, please provide your suggested solution"+"\n")
        file.write("Please answer in the following format:{("+"\n")
        file.write(" \"change_type\":\"Please return expected change or failed change,\""+"\n")
        file.write(" \"reason\":\"Your reason for the change_type field\","+"\n")
