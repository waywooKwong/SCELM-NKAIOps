...
'''
1. classifier = CNNClassifier(class_num=13, length=30) # 初始化分类器

2. pretrained_model_path = "path/to/your/pretrained_model.pt" # 加载预训练模型
classifier.load_model(pretrained_model_path)

3. classifier.fit(X_new, y_new, max_epoch=50, stop_epoch=10, incremental=True) # 进行增量训练

4. incremental_model_path = "path/to/your/incremental_model.pt" # 保存增量训练后的模型
